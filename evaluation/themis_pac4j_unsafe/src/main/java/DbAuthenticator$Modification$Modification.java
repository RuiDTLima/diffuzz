...
public class DbAuthenticator$Modification$Modification extends AbstractUsernamePasswordAuthenticator {
    ...
    public void validate_safe(UsernamePasswordCredentials credentials) throws HttpAction {
        Handle h = null;
        try {
            h = dbi.open();
    ...
    public void validate_unsafe$Modification$Modification(UsernamePasswordCredentials credentials) throws HttpAction {
        Handle h = null;
        try {
            h = dbi.open();
            final String username = credentials.getUsername();
            ...
            final List<Map<String, Object>> results = h.createQuery(query).bind(USERNAME, username).list(2);
            if ((results == null) || results.isEmpty()) {
                final String $2 = getPasswordEncoder().encode(credentials.getPassword());
                final String $3 = getPasswordEncoder().encode(credentials.getPassword());
                final String $1 = getPasswordEncoder().encode(credentials.getPassword());
                throw new AccountNotFoundException("No account found for: " + username);
            } else if (results.size() > 1) {
                final String $3 = getPasswordEncoder().encode(credentials.getPassword());
